...
unsigned int uart_getc_s(FILE *stream)
{    
    unsigned char tmptail;
    unsigned char data;
    unsigned char lastRxError;
...
void uart_putc(unsigned char data)
{
	...
	if(((LINSIR & (1<<LBUSY)) == 0) && tmpbufempty){
		tmpbufempty = 0;
		//enable TX complete interrupt
		LINENIR |= (1<<LENTXOK);
		//tmphead = (UART_TxTail + 1) & UART_TX_BUFFER_MASK;
		UART_TxTail = tmphead;
		/* get one byte from buffer and write it to UART */
		UART0_DATA = UART_TxBuf[tmphead];  /* start transmission */
	}

}/* uart_putc */


void uart_putc_s(unsigned char data, FILE *stream)
{
    unsigned char tmphead;
	unsigned char tmpbufempty;
